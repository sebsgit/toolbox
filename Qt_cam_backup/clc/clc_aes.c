#include <assert.h>
#include <string.h>
#include <stdio.h>
#include "clc_aes.h"

#ifdef CLC_WITH_SSE
	#include "xmmintrin.h"
#endif

static unsigned char clc_s_box[256] = 
{
   
};

static unsigned char clc_s_box_inv[256] = 
{
   
};

static unsigned char clc_E_tab[256] = {
	
};

static unsigned char clc_L_tab[256] = {
	
};

static unsigned char clc_Rcon_tab[16] = {
	0x00, 0x01, 0x02, 0x04, 
	0x08, 0x10, 0x20, 0x40, 
	0x80, 0x1B, 0x36, 0x6C, 
	0xD8, 0xAB, 0x4D, 0x9A
};

static unsigned char clc_get_b_m(const clc_bytes_16 * x, short rn, short cn){
	assert( x && cn>-1 && cn<4 && rn>-1 && rn<4 );
	return x->b[rn+cn*4];
}

static void clc_set_b_m(short rn, short cn, clc_bytes_16 * x, unsigned char v){
	assert( x && cn>-1 && cn<4 && rn>-1 && rn<4 );
	x->b[rn+cn*4] = v;
}

static void clc_xor_16(unsigned char * b1, const unsigned char * b2){
	#ifdef CLC_WITH_SSE
		__m128 bytes_sse = _mm_loadu_ps((const float*)b1);
		__m128 key_sse = _mm_loadu_ps((const float*)b2);
		__m128 xor_sse = _mm_xor_ps(bytes_sse,key_sse);
		_mm_storeu_ps((float *)b1,xor_sse);
	#else
		short i;
		for( i=0 ; i<16 ; ++i ){
			b1[i] ^= b2[i];
		}
	#endif
}


static void clc_sub_bytes( clc_bytes_16 * x ){
	short i;
	for(i=0 ; i<16 ; ++i){
		x->b[i] = clc_s_box[ x->b[i] ];
	}
}

static void clc_sub_bytes_rev( clc_bytes_16 * x ){
	short i;
	for(i=0 ; i<16 ; ++i){
		x->b[i] = clc_s_box_inv[ x->b[i] ];
	}
}

static void clc_shift_row_right(short rn, clc_bytes_16 * x){
	const unsigned short tmp = clc_get_b_m(x,rn,0);
	clc_set_b_m( rn,0,x, clc_get_b_m(x,rn,3) );
	clc_set_b_m( rn,3,x, clc_get_b_m(x,rn,2) );
	clc_set_b_m( rn,2,x, clc_get_b_m(x,rn,1) );
	clc_set_b_m( rn,1,x, tmp );
	
}

static void clc_shift_row_left(short rn, clc_bytes_16 * x){
	const unsigned short tmp = clc_get_b_m(x,rn,0);
	clc_set_b_m( rn,0,x, clc_get_b_m(x,rn,1) );
	clc_set_b_m( rn,1,x, clc_get_b_m(x,rn,2) );
	clc_set_b_m( rn,2,x, clc_get_b_m(x,rn,3) );
	clc_set_b_m( rn,3,x, tmp );
}

static void clc_shift_row_left_n(short rn, clc_bytes_16 * x, short c){
	short i;
	for(i=0 ; i<c ; ++i){
		clc_shift_row_left(rn,x);
	}
}

static void clc_shift_row_right_n(short rn, clc_bytes_16 * x, short c){
	short i;
	for(i=0 ; i<c ; ++i){
		clc_shift_row_right(rn,x);
	}
}

static void clc_shift_row(clc_bytes_16 * x){
	short i;
	for(i=1 ; i<4 ; ++i){
		clc_shift_row_left_n(i,x,i);
	}
}

static void clc_shift_row_rev( clc_bytes_16 * x ){
	short i;
	for(i=1 ; i<4 ; ++i){
		clc_shift_row_right_n(i,x,i);
	}
}

static unsigned char clc_mult_L(unsigned char b1, unsigned char b2){
	unsigned int res = clc_L_tab[b1] + clc_L_tab[b2];
	if( b1 == 0x01 ) return b2;
	if( b2 == 0x01 ) return b1;
	if( b1 == 0x00 || b2 == 0x00 ) return 0x00;
	if( res > 0xFF ){
		res -= 0xFF;
	}
	return clc_E_tab[(unsigned char)res];
}

static void clc_mix_column(short cn, clc_bytes_16 * x){
	unsigned char col[4];
	memcpy(col,(x->b + cn*4),4);
	x->b[0+cn*4] = clc_mult_L(0x02,col[0])^clc_mult_L(0x03,col[1])^clc_mult_L(0x01,col[2])^clc_mult_L(0x01,col[3]);
	x->b[1+cn*4] = clc_mult_L(0x01,col[0])^clc_mult_L(0x02,col[1])^clc_mult_L(0x03,col[2])^clc_mult_L(0x01,col[3]);
	x->b[2+cn*4] = clc_mult_L(0x01,col[0])^clc_mult_L(0x01,col[1])^clc_mult_L(0x02,col[2])^clc_mult_L(0x03,col[3]);
	x->b[3+cn*4] = clc_mult_L(0x03,col[0])^clc_mult_L(0x01,col[1])^clc_mult_L(0x01,col[2])^clc_mult_L(0x02,col[3]);
}

static void clc_mix_column_rev(short cn, clc_bytes_16 * x){
	unsigned char col[4];
	memcpy(col,(x->b + cn*4),4);
	x->b[0+cn*4] = clc_mult_L(0x0E,col[0])^clc_mult_L(0x0B,col[1])^clc_mult_L(0x0D,col[2])^clc_mult_L(0x09,col[3]);
	x->b[1+cn*4] = clc_mult_L(0x09,col[0])^clc_mult_L(0x0E,col[1])^clc_mult_L(0x0B,col[2])^clc_mult_L(0x0D,col[3]);
	x->b[2+cn*4] = clc_mult_L(0x0D,col[0])^clc_mult_L(0x09,col[1])^clc_mult_L(0x0E,col[2])^clc_mult_L(0x0B,col[3]);
	x->b[3+cn*4] = clc_mult_L(0x0B,col[0])^clc_mult_L(0x0D,col[1])^clc_mult_L(0x09,col[2])^clc_mult_L(0x0E,col[3]);
}

static void clc_mix_columns( clc_bytes_16 * x ){
	short i;
	for(i=0 ;i<4 ; ++i){
		clc_mix_column(i,x);
	}
}

static void clc_mix_columns_rev( clc_bytes_16 * x ){
	short i;
	for(i=0 ;i<4 ; ++i){
		clc_mix_column_rev(i,x);
	}
}

static void clc_rot_sub_word(unsigned char* bp, const unsigned char rcon) {
	const unsigned char tmp = bp[0];
	bp[0] = clc_s_box[ bp[1] ] ^ rcon;
	bp[1] = clc_s_box[ bp[2]] ;
	bp[2] = clc_s_box[ bp[3] ];
	bp[3] = clc_s_box[ tmp ];
}

static void clc_key_sched_core( unsigned char * b_in, short i ){
	clc_rot_sub_word(b_in, clc_Rcon_tab[i]);
}

static void clc_add_round_key( clc_bytes_16 * x , const unsigned char * key, short n_round ){
	clc_xor_16(x->b,key+n_round*16);
}

static void clc_add_round_key_rev( clc_bytes_16 * x , const unsigned char * key, short n_round, int key_len ){
	clc_xor_16(x->b,key+key_len-n_round*16);
}

static void clc_encrypt( clc_bytes_16 * x, const unsigned char * key, short n_rounds ){
	short i;
	clc_add_round_key(x,key,0);
	for( i=1 ; i<n_rounds ; ++i ){
		clc_sub_bytes(x);
		clc_shift_row(x);
		clc_mix_columns(x);
		clc_add_round_key(x,key,i);
	}
	clc_sub_bytes(x);
	clc_shift_row(x);
	clc_add_round_key(x,key,n_rounds);
}

static void clc_decrypt( clc_bytes_16 * x, const unsigned char * key, short n_rounds, int key_size ){
	short i;
	clc_add_round_key_rev(x,key,0,key_size);
	for( i=1 ; i<n_rounds ; ++i ){
		clc_shift_row_rev(x);
		clc_sub_bytes_rev(x);
		clc_add_round_key_rev(x,key,i,key_size);
		clc_mix_columns_rev(x);
	}
	clc_shift_row_rev(x);
	clc_sub_bytes_rev(x);
	clc_add_round_key_rev(x,key,n_rounds,key_size);
}

static void clc_init_key(unsigned char * key, const unsigned char * data, int data_len){
	memcpy(key,data,data_len);
}

static void clc_print_key_base(const unsigned char* key, const short size) {
	short i;
	for( i=0 ; i<size ; ++i ){
		printf("%x ",key[i]);
		if(i>0 && i%16 == 0) printf("\n");
	}
	printf("\n");
}

static void clc_init_key_base(unsigned char* key, const unsigned char* data, const int data_len, const short key_len) {
	assert(data_len > 0 && data_len <= key_len && "FAIL: data length incorrect !");
	clc_init_key(key,data,data_len);
	if(data_len < key_len){
		memset(key+data_len, 0, key_len-data_len );
	}
}

void clc_print_key_16( const clc_aes_key_128 * key ){ clc_print_key_base(key->b, 176); }
void clc_print_key_24( const clc_aes_key_192 * key ){ clc_print_key_base(key->b, 208); }
void clc_print_key_32( const clc_aes_key_256 * key ){ clc_print_key_base(key->b, 240); }

void clc_init_key_16( clc_aes_key_128 * key, const unsigned char * data, int data_len ){ clc_init_key_base(key->b, data, data_len, 16); }
void clc_init_key_24( clc_aes_key_192 * key, const unsigned char * data, int data_len ){ clc_init_key_base(key->b, data, data_len, 24); }
void clc_init_key_32( clc_aes_key_256 * key, const unsigned char * data, int data_len ){ clc_init_key_base(key->b, data, data_len, 32); }

void clc_expand_key_16( clc_aes_key_128 * key ){
	unsigned char buff[4];
	unsigned char p = 16;
	unsigned short i=1;
	unsigned char a;
	while(p < 176) {
		memcpy(buff, (key->b+p-4), 4);
		if(p % 16 == 0){
			clc_key_sched_core(buff,i);
			i++;
		}
		for(a = 0; a < 4; a++) {
			key->b[p] = key->b[p - 16] ^ buff[a];
			++p;
		}
	}
}

void clc_expand_key_24( clc_aes_key_192 * key ){
	unsigned char buff[4];
	unsigned char p = 24;
	unsigned short i=1;
	unsigned char a;
	while(p < 208) {
		memcpy(buff, (key->b+p-4), 4);
		if(p % 24 == 0){
			clc_key_sched_core(buff,i);
			i++;
		}
		for(a = 0; a < 4; a++) {
			key->b[p] = key->b[p - 24] ^ buff[a];
			++p;
		}
	}
}

void clc_expand_key_32( clc_aes_key_256 * key ){
	unsigned char buff[4];
	unsigned char p = 32;
	unsigned short i=1;
	unsigned char a;
	while(p < 240) {
		memcpy(buff, (key->b+p-4), 4);
		if(p % 32 == 0){
			clc_key_sched_core(buff,i);
			i++;
		}
		if(p % 32 == 16) {
			for(a = 0; a < 4; a++) 
				buff[a] = clc_s_box[ buff[a] ];
		}
		for(a = 0; a < 4; a++) {
			key->b[p] = key->b[p - 32] ^ buff[a];
			++p;
		}
	}
}

void clc_encrypt_16( clc_bytes_16 * x, const clc_aes_key_128 * key ){ clc_encrypt(x,key->b,10); }
void clc_decrypt_16( clc_bytes_16 * x, const clc_aes_key_128 * key ){ clc_decrypt(x,key->b,10,160); }
void clc_encrypt_24( clc_bytes_16 * x, const clc_aes_key_192 * key ){ clc_encrypt(x,key->b,12); }
void clc_decrypt_24( clc_bytes_16 * x, const clc_aes_key_192 * key ){ clc_decrypt(x,key->b,12,192); }
void clc_encrypt_32( clc_bytes_16 * x, const clc_aes_key_256 * key ){ clc_encrypt(x,key->b,14); }
void clc_decrypt_32( clc_bytes_16 * x, const clc_aes_key_256 * key ){ clc_decrypt(x,key->b,14,224); }
