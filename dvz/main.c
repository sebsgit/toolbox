...
int main(int argc, char ** argv){
	...
	dvz_node_id_t node4 = dvz_register_node(ctx, "secondout", 0);
	dvz_connect(ctx, node1, 0, node3, 0, DVZ_CONNECT_SOURCE);
	dvz_connect(ctx, node3, 0, node2, 0, DVZ_CONNECT_SOURCE);
	dvz_connect(ctx, node3, 1, node4, 0, DVZ_CONNECT_SOURCE);
