...
ssize_t size_show(struct device *dev, struct device_attribute *attr,
                    char *buf)
{
    /* dev->parent has been assigned in the probe function */
    const fakedrv_dev_local_data_t * drv_data = (const fakedrv_dev_local_data_t *)dev->parent->driver_data;
    if (!drv_data)
    {
        pr_err("Invalid driver_data ptr!\n");
        return -EINVAL;
    }

    return sprintf(buf, "%u", drv_data->plt_data.size);
}

ssize_t access_mode_show(struct device *dev, struct device_attribute *attr,
                    char *buf)
{
    const fakedrv_dev_local_data_t * drv_data = (const fakedrv_dev_local_data_t *)dev->parent->driver_data;
    if (!drv_data)
    ...
    return sprintf(buf, "%d", drv_data->plt_data.access_mode);
}

ssize_t access_mode_store(struct device *dev, struct device_attribute *attr,
                    const char *buf, size_t count)
{
    ...
    if (result != FAKEDRV_RW && result != FAKEDRV_R)
    {
        pr_err("Accepted values for access_mode are: %d and %d\n", FAKEDRV_RW, FAKEDRV_R);
        return -EINVAL;
    }

    fakedrv_dev_local_data_t * drv_data = (fakedrv_dev_local_data_t *)dev->parent->driver_data;
    if (!drv_data)
    {
        pr_err("Invalid driver_data ptr!\n");
        return -EINVAL;
    }
    drv_data->plt_data.access_mode = (int)result;

    return count;
}

static const DEVICE_ATTR_RO(size);
static const DEVICE_ATTR(access_mode, S_IWUSR | S_IRUGO, access_mode_show, access_mode_store);
...
static int fakedrv_probe(struct platform_device * platform_dev)
{
    ...
    pr_info("probe: assign %p to dev ptr %p\n", new_entry, &platform_dev->dev);
    ...
    new_entry->device_p = device_create(fakedrv_data.class_p, &platform_dev->dev, fakedrv_data.device_number_base + new_entry->id, NULL, DRV_DEV_NAME, new_entry->id);
    ...
    res = device_create_file(new_entry->device_p, &dev_attr_access_mode);
    if (res != 0)
    {
        pr_err("Cannot create device file for 'access_mode': %d\n", res);
        goto cdev_delete;
    }

    res = device_create_file(new_entry->device_p, &dev_attr_size);
    if (res != 0) 
    {
        pr_err("Cannot create device file for 'size': %d\n", res);
        device_remove_file(new_entry->device_p, &dev_attr_access_mode);
        goto cdev_delete;
    }
...
static int fakedrv_remove(struct platform_device * pdev)
{
    ...
    device_remove_file(&pdev->dev, &dev_attr_access_mode);
    device_remove_file(&pdev->dev, &dev_attr_size);
