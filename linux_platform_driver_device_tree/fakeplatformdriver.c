...
/*
Reads the properties of the platform device from the device tree
*/
static fakedrv_platform_device_data_t * read_platform_data_from_dt(struct device *dev)
{
    if (!dev->of_node)
    {
        dev_warn(dev, "No OF_NODE found\n");
        return NULL;
    }
    ...
    int res = of_property_read_string(dev->of_node, "org,serial", &serial_no);
    if (res != 0)
    {
        dev_err(dev, "Can't read serial: %d\n", res);
        return NULL;
    }
    res = of_property_read_u32(dev->of_node, "org,size", &size);
    if (res != 0)
    {
        dev_err(dev, "Can't read size: %d\n", res);
        return NULL;
    }
    res = of_property_read_s32(dev->of_node, "org,access_mode", &access_mode);
    if (res != 0)
    {
        dev_err(dev, "Can't read access_mode: %d\n", res);
        return NULL;
    }

    fakedrv_platform_device_data_t *ptr = devm_kzalloc(dev, sizeof(fakedrv_platform_device_data_t), GFP_KERNEL);
    memcpy(ptr->serial_no, serial_no, strlen(serial_no));
    ptr->access_mode = access_mode;
    ptr->size = size;
    return ptr;
}
...
{
    int res = 0;
    dev_info(&platform_dev->dev, "probe");

    const fakedrv_platform_device_data_t * dev_data_p = read_platform_data_from_dt(&platform_dev->dev);
    if (!dev_data_p)
    {
        dev_err(&platform_dev->dev, "no valid platform data found");
        return -ENOENT;
    }
    ...
    if (!new_entry)
    {
        dev_err(&platform_dev->dev, "Cannot allocate fakedrv_dev_local_data_t");
        return -ENOMEM;
    }
    ...
    /*
    returns the .data associated with the device, which we defined in the match table below
    */
    new_entry->id = (int)of_device_get_match_data(&platform_dev->dev);

    dev_info(&platform_dev->dev, "new dev: %d, size: %d, serial: %s, access mode: %d\n",
        new_entry->id, 
    ...
    if (!new_entry->buffer)
    {
        dev_err(&platform_dev->dev, "Cannot allocate device buffer");
        res = -ENOMEM;
        goto new_entry_free;
    }
    ...
    if (add_res < 0)
    {
        dev_err(&platform_dev->dev, "Cannot add cdev: %d\n", add_res);
        res = add_res;
...
static struct of_device_id fakedrv_devs_id[] = {
    [0] = {.compatible = FAKEDRV_PLATFORM_NAME, .data = (void*)0},
    [1] = {.compatible = FAKEDRV_PLATFORM_NAME_VARIANT_A, .data = (void*)1},
    [2] = {.compatible = FAKEDRV_PLATFORM_NAME_VARIANT_B, .data = (void*)2},
    {}
};
...
static struct platform_driver fakedrv_platform_driver = {
    .driver = {
        ...
        .of_match_table = of_match_ptr(fakedrv_devs_id) // when CONFIG_OF is false, the of_match_ptr returns NULL
    },
    .probe = fakedrv_probe,
